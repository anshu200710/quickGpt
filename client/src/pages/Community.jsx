...
const Community = () => {
  ...
  return (
    <div className='p-6 pt-12 xl:px-12 2xl:px-20 w-full mx-auto h-full overflow-y-scroll'>
      ...
      {images.length > 0  ? (
        <div className="flex flex-wrap max-sm:justify-center gap-5">
          {images.map((item, index)=> (
            <a key={index} href={item.imageUrl} target='_blank' className='relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-transform duration-300 ease-in-out'>
      ...
      (<p className='text-center text-gray-600 dark:text-purple-200 mt-20'>
        No Images Available
      </p>)} 
    </div>
  )
